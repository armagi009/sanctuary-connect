...
import { Star, ShieldCheck, MapPin, Clock, Target, Lightbulb, Loader2 } from 'lucide-react';
        ...
        <div className="py-8 md:py-12">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 lg:gap-12">
            ...
            <div className="md:col-span-2 space-y-8">
              <Card>
                <CardHeader><CardTitle>My Philosophy</CardTitle></CardHeader>
                <CardContent><p className="text-muted-foreground leading-relaxed">{practitioner.philosophy}</p></CardContent>
              </Card>
              <Card>
                <CardHeader><CardTitle>How I Can Help</CardTitle></CardHeader>
                <CardContent className="space-y-6">
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center"><Target className="h-5 w-5 text-primary" /></div>
                    <div>
                  ...
                  <div className="flex items-start gap-4">
                    <div className="flex-shrink-0 h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center"><Lightbulb className="h-5 w-5 text-primary" /></div>
                    <div>
              ...
              <Card>
                <CardHeader><CardTitle>Modalities</CardTitle></CardHeader>
                <CardContent className="flex flex-wrap gap-2">
                  {practitioner.modalities.map(modality => <Badge key={modality} variant="outline" className="text-base py-1 px-3">{modality}</Badge>)}
                </CardContent>
              </Card>
              <Card>
                <CardHeader><CardTitle>Certifications & Experience</CardTitle></CardHeader>
                <CardContent>
                  <ul className="space-y-4">
                    {practitioner.certifications.map((cert, index) => (
                      <li key={index} className="flex items-start gap-4">
                        <div className="flex-shrink-0 h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center"><ShieldCheck className="h-5 w-5 text-primary" /></div>
                        <div>
              ...
              <Card>
                <CardHeader><CardTitle>Session Offerings</CardTitle></CardHeader>
                  ...
                  {MOCK_SESSIONS.map((session, index) => (
                    <div key={index} className="flex justify-between items-start p-4 border rounded-lg bg-background">
                      ...
                      <div className="text-right">
                        ...
                        <Button asChild size="sm" className="mt-1"><Link to={`/book/${practitioner.id}`} state={{ session }}>Book Now</Link></Button>
                      </div>
                    </div>
                  ))}
                </CardContent>
              ...
              <Card>
                <CardHeader><CardTitle>Reviews & Testimonials ({reviews.length})</CardTitle></CardHeader>
                <CardContent className="space-y-6">
                  {reviews.length > 0 ? reviews.map(review => <ReviewCard key={review.id} review={review} />) : <p className="text-muted-foreground text-center py-4">No reviews yet.</p>}
                </CardContent>
              </Card>
            </div>
          </div>
